...
def main(config):
    ...
    pgd = PGD(target_model, norm, device, 4/255.0, 1/255.0, num_iter=10, targeted=True, random_start=True)  
        ...
        for eps in epsilon_list:
            ...
            with torch.no_grad():
                ...
                with torch.enable_grad():
                    adv_images = pgd.attack(adv_images, logits.argmax(1))
    ...
    as_list_filtered = {eps: [] if eps > 0 else as_list[eps] for eps in epsilon_list}
    for eps in epsilon_list:
        filter = (1-acc[eps])*acc[0]
        as_list_filtered[eps] = as_list[eps][filter]
    ...
    mean_as_filtered = [as_list_filtered[eps].mean() for eps in epsilon_list]
    std_as_filtered = [as_list_filtered[eps].std() for eps in epsilon_list]
    ...
    ax1.plot(epsilon_list, mean_as_filtered, label='Anomaly Score Filtered (mean)', linestyle='-', color='C2')
    ax1.fill_between(
        epsilon_list,
        [m - s for m, s in zip(mean_as_filtered, std_as_filtered)],
        [m + s for m, s in zip(mean_as_filtered, std_as_filtered)],
        alpha=0.3,
        label='Anomaly Score Filtered(std)',
        color='C2'
    )
